...
temp_counts_matrix <- temp_object@assays$RNA@counts
temp_non_zero_genes <- rowSums(temp_counts_matrix != 0) > 0
DefaultAssay(temp_object) <- "RNA" 
temp_object <- subset(temp_object, features = rownames(temp_counts_matrix)[temp_non_zero_genes])

start_time <- Sys.time()
mem <- peakRAM({
  Celda_cluster_result <- celda_C(x = temp_object@assays$RNA@counts, K = n_cluster, seed = benchmark_seed)
})
