...
mem <- peakRAM({
  sce <- SingleCellExperiment(
    assays = list(
      counts = as.matrix(temp_object@assays$RNA@counts),
      logcounts = log2(as.matrix(temp_object@assays$RNA@counts) + 1)
    )
  )
  ...
  sce <- sc3(sce,ks = n_cluster:n_cluster,svm_max=dim(temp_object@assays$RNA@counts)[2],rand_seed=benchmark_seed)
})
