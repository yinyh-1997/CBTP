...
method <- "DEPECHER-integration_feature"

start_time <- Sys.time()
mem <- peakRAM({
  depeche_cluster_result<-depeche(t(as.matrix(temp_object@assays$ADT@data)))
})
...
pred_label <- depeche_cluster_result$clusterVector
unique(pred_label)
...
detach("package:DepecheR", unload=TRUE)
