...
temp_CDC_RAN_object <- RunUMAP(temp_object, reduction = "pca_rna", dims = 1:20, assay = "RNA")

start_time <- Sys.time()
mem <- peakRAM({
  CDC_cluster_result <- CDC(temp_CDC_RAN_object@reductions$umap@cell.embeddings, k, ratio)
})
