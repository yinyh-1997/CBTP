...
method <- "SHARP-integration_feature"

start_time <- Sys.time()
mem <- peakRAM({
  SHARP_cluster_result = SHARP(temp_object@assays$ADT@data, N.cluster = n_cluster, rN.seed = benchmark_seed, prep=TRUE)
})
...
pred_label <- SHARP_cluster_result$pred_clusters
...
detach("package:SHARP", unload=TRUE)
