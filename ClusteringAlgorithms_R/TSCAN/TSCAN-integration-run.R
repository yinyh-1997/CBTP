...
method <- "TSCAN-integration_feature"

start_time <- Sys.time()
mem <- peakRAM({
  TSCAN_cluster_result <- exprmclust(as.matrix(temp_object@assays$ADT@data),clusternum=n_cluster)
})
...
pred_label <- TSCAN_cluster_result$clusterid
...
detach("package:TSCAN", unload=TRUE)
