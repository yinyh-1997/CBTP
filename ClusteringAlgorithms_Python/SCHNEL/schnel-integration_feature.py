...
unique, inverse = np.unique(clusters[0], return_inverse=True)
schnel_labels = unique[inverse]
true_label = np.array(y)
pred_label = np.array(schnel_labels)

acc, purity, nmi, ari= evaluate(true_label,pred_label)
type = "integration_feature"
method_name = "SCHNEL-integration_feature-data"
