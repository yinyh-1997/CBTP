...
save_dir = dataset_name+'_'+time_str

tracemalloc.start()
start_time = get_time()

adata=desc.train(adata,save_dir=str(save_dir),use_GPU=True,GPU_id=1,random_seed=benchmark_randomseed)
...
desc_labels = adata.obs['desc_0.8']
true_label = np.array(y)
pred_label = np.array(desc_labels)

acc, purity, nmi, ari= evaluate(true_label,pred_label)
type = "integration_feature"
method_name = "DESC-integration_feature"
